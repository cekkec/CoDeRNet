...
## Default Architecture (paper configuration)

These settings are baked into `train.py` / `test.py` to keep the release surface minimal:

| Component | Setting |
|---|---|
| Backbone | `tu-convnext_base`, ImageNet-pretrained |
| Fusion | signed: `[F_t0, F_t1, |Δ|, Δ]` |
| Branch projection | per-scale 1×1 + BN + ReLU + 3×3 + BN + ReLU |
| CoDeR — variant | `light` (1×1 adapter) |
| CoDeR — direction | bidirectional |
| CoDeR — seg-recv scale | `1/8`  (32×32 on 256² input) |
| CoDeR — cap-recv scale | `1/16` (16×16) |
| Seg decoder | FPN top-down + 2 ResBlock refinement + 2× ConvTranspose |
| Cap decoder | 1-layer transformer (8 heads); memory = global + meso (2×2) + local |
| Loss | `CE + Dice` (seg) + token-level CE (cap); stop-grad-normalized sum |
